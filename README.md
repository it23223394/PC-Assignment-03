...
POSIX / WSL:
```bash
export OMP_NUM_THREADS=4; ./openmp_exec
```

PowerShell (Windows):
```powershell
$env:OMP_NUM_THREADS=4; ./openmp_exec
```
...
Make (if MPI toolchain present):
```bash
cd source_code/mpi && make
```
...
Notes: Replace `4` with the number of processes you want to test. Default `N` is `800` in `mpi_matmul.c`. On Windows, consider using MS-MPI or WSL.

### CUDA

Prerequisites: NVIDIA GPU, correct drivers, and the CUDA Toolkit (`nvcc`).

Make:
```bash
cd source_code/cuda && make
```
...
Arguments: `N [block_x] [block_y]` — matrix size and block dimensions. If omitted, defaults are used (the example CUDA program defaults to `N=1024`).
...
- On Windows, prefer WSL (Windows Subsystem for Linux) for the MPI build/run workflow. For CUDA use the native CUDA toolkit with an NVIDIA GPU and up-to-date drivers.
...
The repository was tested on Google Colab for convenience. Colab is useful for quick experimentation but has limits (ephemeral VMs, limited GPU memory, and single-node MPI only).

Below are example notebook cells you can run in Colab to build and test each implementation.
